...
class Bloxorz:
 ...
 def successorSingleBlockX2Y2(self, currentState: State, listVisited, matrixMap):
  listSuccessor = []
  stateRight = self.checkCombine(
   State(currentState.x1, currentState.y1, currentState.x2, currentState.y2 + 1, currentState.oriented, listVisited,
  ...
  stateTop = self.checkCombine(
   State(currentState.x1, currentState.y1, currentState.x2 - 1, currentState.y2, currentState.oriented, listVisited,
         matrixMap, currentState))
  listSuccessor += self.addListSuccessor(listVisited, stateTop)
 ...
 def checkCombine(self, newState: State):
  if newState.x1 == newState.x2:
   if newState.y1 - newState.y2 == -1:
    return State(newState.x1, newState.y1, newState.x2, newState.y2, 2, newState.listVisited, newState.matrixMap,
                 newState.parent)
   if newState.y1 - newState.y2 == 1:
    return State(newState.x2, newState.y2, newState.x1, newState.y1, 2, newState.listVisited, newState.matrixMap,
                 newState.parent)
  if newState.y1 == newState.y2:
   if newState.x1 - newState.x2 == -1:
    return State(newState.x1, newState.y1, newState.x2, newState.y2, 1, newState.listVisited, newState.matrixMap,
                 newState.parent)
   if newState.x1 - newState.x2 == 1:
    return State(newState.x2, newState.y2, newState.x1, newState.y1, 1, newState.listVisited, newState.matrixMap,
                 newState.parent)
  return newState
 ...
 def successor(self, currentState: State, listVisited, matrixMap):
  ...
  if currentState.oriented == 0:  # Column is Vertical
   listSuccessor += self.addListSuccessor(listVisited, State(currentState.x1, currentState.y1 + 1, currentState.x1,
                                                             currentState.y1 + 2, 2, listVisited, matrixMap,
                                                             currentState))  # stateRight
   listSuccessor += self.addListSuccessor(listVisited, State(currentState.x1, currentState.y1 - 2, currentState.x1,
                                                             currentState.y1 - 1, 2, listVisited, matrixMap,
                                                             currentState))  # stateLeft
   listSuccessor += self.addListSuccessor(listVisited, State(currentState.x1 - 2, currentState.y1, currentState.x1 - 1,
                                                             currentState.y1, 1, listVisited, matrixMap,
                                                             currentState))  # stateTop
   listSuccessor += self.addListSuccessor(listVisited, State(currentState.x1 + 1, currentState.y1, currentState.x1 + 2,
                                                             currentState.y1, 1, listVisited, matrixMap,
   ...
   if currentState.oriented == 2:  # 2:song song Y(--);
    listSuccessor += self.addListSuccessor(listVisited,
                                           State(currentState.x2, currentState.y1 + 2, -1, -1, 0, listVisited,
                                                 matrixMap, currentState))  # stateRight
    listSuccessor += self.addListSuccessor(listVisited,
                                           State(currentState.x1, currentState.y1 - 1, -1, -1, 0, listVisited,
                                                 matrixMap, currentState))  # stateLeft
    listSuccessor += self.addListSuccessor(listVisited, State(currentState.x1 - 1, currentState.y1, currentState.x2 - 1,
   ...
   if currentState.oriented == 1:  # 1:song song truc x(|)
    listSuccessor += self.addListSuccessor(listVisited, State(currentState.x1, currentState.y1 + 1, currentState.x2,
    ...
    listSuccessor += self.addListSuccessor(listVisited, State(currentState.x1, currentState.y1 - 1, currentState.x2,
                                                              ...
                                                              currentState))  # stateLeft
    listSuccessor += self.addListSuccessor(listVisited,
                                           State(currentState.x1 - 1, currentState.y1, -1, -1, 0, listVisited,
                                                 matrixMap, currentState))  # stateTop
    listSuccessor += self.addListSuccessor(listVisited,
                                           State(currentState.x2 + 1, currentState.y2, -1, -1, 0, listVisited,
 ...
 def enterSpecialSquare(self, stateCheck: State):
  ...
  if stateCheck.oriented == 0:
   if stateCheck.matrixMap[stateCheck.x1][stateCheck.y1] == 2 or stateCheck.matrixMap[stateCheck.x1][
    ...
    for x in self.specialSquareList:
     if x.xLocate == stateCheck.x1 and x.yLocate == stateCheck.y1:
      stateCheck.x1, stateCheck.y1, stateCheck.x2, stateCheck.y2 = x.Open[0][0], x.Open[0][1], x.Open[1][0], x.Open[1][
       1]
...
def main():
 ...
 if stage == 'Stage/Stage1.txt':
  ...
  bloxorz = Bloxorz(mapMatrix, [specialSquare(4, 4, [(6, 6), (6, 7)], 3), specialSquare(3, 10, [(6, 12), (6, 13)], 2)],
                    State(6, 3, -1, -1, 0, [], mapMatrix, None), State(3, 15, -1, -1, 0, [], mapMatrix, None))  # Stage2
  ...
  bloxorz = Bloxorz(mapMatrix, [specialSquare(3, 14, [(3, 11), (3, 14)], 4), specialSquare(11, 7, [(3, 5), (3, 6)], 3),
                                specialSquare(11, 13, [(3, 8), (3, 9), (4, 14), (5, 14)], 2)],
                    State(3, 11, -1, -1, 0, [], mapMatrix, None), State(3, 3, -1, -1, 0, [], mapMatrix, None))  # Stage8
  ...
  bloxorz = Bloxorz(mapMatrix, [specialSquare(5, 14, [(4, 3), (4, 4)], 2), specialSquare(11, 15, [(5, 3), (5, 4)], 2)],
  ...
  bloxorz = Bloxorz(mapMatrix, [specialSquare(9, 13, [(10, 11), (10, 12)], 3, 2),
                                ...
                                specialSquare(7, 9, [(10, 3), (3, 15)], 4), specialSquare(5, 10, [(3, 7), (3, 8)], 3),
                                specialSquare(5, 10, [(3, 12), (3, 13)], 3), specialSquare(3, 14, [(4, 4), (4, 5)], 2),
                                specialSquare(3, 14, [(3, 7), (3, 8)], 2)],
                    State(10, 3, -1, -1, 0, [], mapMatrix, None), State(10, 14, -1, -1, 0, [], mapMatrix, None))
 elif stage == 'Stage/Stage16.txt':
  bloxorz = Bloxorz(mapMatrix, [specialSquare(7, 11, [(3, 2), (2, 3)], 4), specialSquare(3, 4, [(3, 2), (3, 4)], 4),
  ...
  bloxorz = Bloxorz(mapMatrix,
                    [specialSquare(5, 3, [(5, 10), (5, 11)], 3, 2), specialSquare(3, 4, [(10, 3), (10, 4)], 3, 2),
                     ...
                     specialSquare(8, 10, [(10, 3), (10, 4)], 3, 1), specialSquare(8, 10, [(5, 14), (5, 15)], 3, 1),
  ...
  bloxorz = Bloxorz(mapMatrix,
                    [specialSquare(2, 12, [(7, 9), (7, 10)], 3), specialSquare(7, 12, [(11, 4), (11, 5)], 3, 2),
                     specialSquare(11, 12, [(11, 4), (11, 5)], 3, 1)], State(2, 3, -1, -1, 0, [], mapMatrix, None),
                    State(8, 3, -1, -1, 0, [], mapMatrix, None))
 elif stage == 'Stage/Stage20.txt':
  bloxorz = Bloxorz(mapMatrix, [specialSquare(9, 3, [(3, 12), (3, 13)], 3), specialSquare(6, 5, [(3, 7), (3, 8)], 3, 2),
  ...
  bloxorz = Bloxorz(mapMatrix, [specialSquare(5, 15, [(11, 8), (11, 11)], 2, 2), specialSquare(7, 14, [(11, 8)], 3, 1),
  ...
  bloxorz = Bloxorz(mapMatrix,
                    [specialSquare(8, 14, [(5, 3), (5, 4)], 3, 1), specialSquare(11, 14, [(5, 14), (5, 15)], 3, 1),
                     specialSquare(11, 14, [(2, 5), (2, 6), (2, 12), (2, 13), (8, 12), (8, 13)], 3, 2),
                     specialSquare(5, 2, [(10, 5), (10, 6)], 2, 1), specialSquare(5, 2, [(11, 12), (11, 13)], 2, 2),
                     specialSquare(2, 4, [(2, 12), (2, 13)], 2, 1), specialSquare(2, 4, [(8, 12), (8, 13)], 2, 2),
                     specialSquare(2, 14, [(7, 7), (7, 8)], 2, 1), specialSquare(5, 16, [(11, 5)], 2, 1)],
                    State(5, 9, -1, -1, 0, [], mapMatrix, None), State(10, 3, -1, -1, 0, [], mapMatrix, None))
 elif stage == 'Stage/Stage30.txt':
  bloxorz = Bloxorz(mapMatrix, [specialSquare(9, 14, [(9, 16)], 2), specialSquare(4, 16, [(8, 11), (8, 14)], 2, 1),
  ...
  bloxorz = Bloxorz(mapMatrix,
                    [specialSquare(9, 9, [(9, 6), (9, 7), (9, 11), (9, 12), (4, 6), (4, 7), (4, 11), (4, 12)], 3, 2),
                     specialSquare(6, 8, [(9, 6), (9, 7), (9, 11), (9, 12), (4, 6), (4, 7), (4, 11), (4, 12)], 3, 2),
  ...
  bloxorz = Bloxorz(mapMatrix, [specialSquare(4, 13, [(10, 4), (10, 5)], 2),
                                ...
                                specialSquare(9, 7, [(4, 6), (4, 7)], 2)], State(8, 12, -1, -1, 0, [], mapMatrix, None),
